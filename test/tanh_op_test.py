...
class TanhOpTest(MUSATestCase):
  """Tests for MUSA Tanh operator."""

  def _test_tanh(self, input_data, dtype, rtol=1e-5, atol=1e-8):
    """Test Tanh operation with given input data and dtype."""
    ...
    with tf.device('/CPU:0'):
      cpu_result = tf.nn.tanh(x)
    
    # Test on MUSA
    with tf.device('/device:MUSA:0'):
      musa_result = tf.nn.tanh(x)
  ...
  def testTanhBasic(self):
    """Basic Tanh test with mixed positive/negative values."""
    test_data = [-2.0, -1.0, 0.0, 1.0, 2.0]
    for dtype in [tf.float32, tf.float16, tf.bfloat16 ]:  
      rtol = 1e-2 if dtype in [tf.float16, tf.bfloat16] else 1e-5
      atol = 1e-2 if dtype in [tf.float16, tf.bfloat16] else 1e-8
      self._test_tanh(test_data, dtype, rtol=rtol, atol=atol)
  ...
  def testTanhZeroInput(self):
    """Tanh test with zero input."""
