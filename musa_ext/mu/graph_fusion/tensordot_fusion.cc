...
std::vector<int> ExtractAxesFromGather(const GraphDef& graph,
  ...
  const NodeDef* indices_const =
      GetConstLikeNode(graph, gather_node->input(1));
...
FusionMatchResult MusaTensorDotFusion::Match(const GraphDef& graph,
  ...
  if (IsOp(start_node, "Reshape")) {
    VLOG(2) << "[TensorDot::Match] ENTER Reshape path, node=" << start_node.name();
...
FusionMatchResult MusaTensorDotFusion::MatchFromReshapeNode(
  ...
  if (!IsOp(reshape_2, "Reshape")) {
    VLOG(2) << "[TensorDot::Match] FAIL: not Reshape op, node=" << reshape_2.name();
    return result;
  }
  ...
  if (reshape_2.name() != tensordot_prefix) {
    VLOG(2) << "[TensorDot::Match] FAIL: Reshape_2 name != tensordot_prefix, name="
  ...
  if (!matmul_node || !IsOp(*matmul_node, "MatMul")) {
    VLOG(2) << "[TensorDot::Match] FAIL step1: input[0] is not MatMul, actual="
              << (matmul_node ? matmul_node->op() : "NULL") << ", node=" << reshape_2.name();
    return result;
  }
  ...
  if (!concat_node || !IsOp(*concat_node, "ConcatV2")) {
    VLOG(2) << "[TensorDot::Match] FAIL step1: input[1] is not ConcatV2, actual="
              << (concat_node ? concat_node->op() : "NULL") << ", node=" << reshape_2.name();
    return result;
  }
  ...
  if (!r_in0 || !r_in1) {
    VLOG(2) << "[TensorDot::Match] FAIL step3: cannot find Reshape_1 inputs, node="
              << reshape_2.name();
    return result;
  }
  ...
  if (IsOp(*r_in0, "Pack") && BelongsToTensordot(r_in0->name(), tensordot_prefix)) {
    pack_node = r_in0;
    tensor_input_node = r_in1;
    tensor_input_idx = 1;
  } else if (IsOp(*r_in1, "Pack") && BelongsToTensordot(r_in1->name(), tensordot_prefix)) {
    pack_node = r_in1;
  ...
  if (!prod_1 || !prod_2 || !IsOp(*prod_1, "Prod") || !IsOp(*prod_2, "Prod")) {
    VLOG(2) << "[TensorDot::Match] FAIL step4: Pack inputs not both Prod, in0.op="
              << (prod_1 ? prod_1->op() : "NULL") << ", in1.op="
              << (prod_2 ? prod_2->op() : "NULL") << ", node=" << reshape_2.name();
    return result;
  }
  ...
  if (!gather_1 || !prod1_red) {
    VLOG(2) << "[TensorDot::Match] FAIL step4: Prod_1 pattern mismatch, node=" << reshape_2.name();
    return result;
  }
  ...
  if (!gather_2 || !prod2_red) {
    VLOG(2) << "[TensorDot::Match] FAIL step4: Prod_2 pattern mismatch, node=" << reshape_2.name();
    return result;
  }
  ...
  if (!shape_1) {
    VLOG(2) << "[TensorDot::Match] FAIL step4: GatherV2_1 params is not Shape, node="
              << reshape_2.name();
    return result;
  }
  ...
  if (!shape_1_b || shape_1_b != shape_1) {
    VLOG(2) << "[TensorDot::Match] FAIL step4: GatherV2_2 params not same Shape_1, shape_1="
              << shape_1->name() << ", shape_1_b="
              << (shape_1_b ? shape_1_b->name() : "NULL") << ", node=" << reshape_2.name();
    return result;
  }
  ...
  if (!GetConstLikeNode(graph, gather_1->input(1)) ||
    ...
    VLOG(2) << "[TensorDot::Match] FAIL step4: GatherV2 indices/axis not const-like, node="
              << reshape_2.name();
    return result;
  }
  ...
  // =========================================================================
  // 第5步: ConcatV2（已验证属于前缀）至少包含 GatherV2_1，其余 Const；axis Const
  // =========================================================================
  if (concat_node->input_size() < 3) {
    VLOG(2) << "[TensorDot::Match] FAIL step5: ConcatV2 input_size="
              << concat_node->input_size() << " (need >=3), node=" << reshape_2.name();
    return result;
  }

  const NodeDef* concat_axis =
      GetConstLikeNode(graph, concat_node->input(concat_node->input_size() - 1));
  if (!concat_axis) {
    VLOG(2) << "[TensorDot::Match] FAIL step5: ConcatV2 axis not const-like, node="
              << reshape_2.name();
    return result;
  }
  ...
  if (!has_gather1_in_concat) {
    VLOG(2) << "[TensorDot::Match] FAIL step5: ConcatV2 missing GatherV2_1, node="
              << reshape_2.name();
    return result;
  }
  ...
  for (const auto& node_name : nodes_to_fuse) {
    result.captured_attrs["fuse_node_" + std::to_string(
        result.captured_attrs.size())] = node_name;
  }
...
Status MusaTensorDotFusion::Apply(GraphDef* graph,
  ...
  if (!IsKernelAvailable()) {
    VLOG(2) << "[TensorDot::Apply] RETURN: kernel not available, skipping fusion";
    return Status::OK();
  }
  ...
  if (output_it == match_result.captured_nodes.end()) {
    VLOG(2) << "[TensorDot::Apply] RETURN: missing output node in captured_nodes";
  ...
  for (const auto& node : graph->node()) {
    if (node.name() == output_name && node.op() == "MusaTensorDot") {
      VLOG(2) << "[TensorDot::Apply] RETURN: already fused, node=" << output_name;
  ...
  } else if (weight_it != match_result.captured_nodes.end() && weight_it->second) {
