...
template <typename Scalar>
void MusaMatrixSetDiagKernelLauncher(
    musaStream_t stream, typename TTypes<Scalar, 3>::ConstTensor& input,
    typename TTypes<Scalar, 3>::ConstTensor& diag,
    typename TTypes<Scalar, 3>::Tensor& output,
    const Eigen::Index lower_diag_index, const Eigen::Index upper_diag_index,
    const Eigen::Index max_diag_len, const bool left_align_superdiagonal,
    const bool left_align_subdiagonal) {
  ...
  const int num_threads = batch_size * num_diags * max_diag_len;
  const int grid_size = (num_threads + block_size - 1) / block_size;
  if (input.data() == output.data()) {
    // in `replace in-place` case
    MatrixSetDiagKernel<Scalar><<<grid_size, block_size, 0, stream>>>(
        batch_size * num_diags * max_diag_len, m, n, num_diags, max_diag_len,
        upper_diag_index, left_align_superdiagonal, left_align_subdiagonal,
        diag.data(), output.data());
  } else {
    // MatrixCopyInputAndSetDiagKernel
    MatrixCopyInputAndSetDiagKernel<Scalar>
        <<<grid_size, block_size, 0, stream>>>(
            batch_size * m * n, m, n, num_diags, max_diag_len, lower_diag_index,
            upper_diag_index, left_align_superdiagonal, left_align_subdiagonal,
            input.data(), diag.data(), output.data());
  }
}

#define INSTANTIATE_MATRIX_SET_DIAG_KERNEL(Scalar)                                  \
  template void MusaMatrixSetDiagKernelLauncher<Scalar>(                                       \
...
INSTANTIATE_MATRIX_SET_DIAG_KERNEL(int64_t);
INSTANTIATE_MATRIX_SET_DIAG_KERNEL(Eigen::half);
INSTANTIATE_MATRIX_SET_DIAG_KERNEL(Eigen::bfloat16);
