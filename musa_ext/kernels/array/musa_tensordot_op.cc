...
class MusaTensorDotOp : public MusaOpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    VLOG(2) << "MusaTensorDot: output shape = " << output->shape().DebugString();
  }
 ...
 private:
  ...
  Status DoTensorDot(OpKernelContext* ctx, const Tensor& a, const Tensor& b,
    ...
    Tensor a_2d;
    TF_RETURN_IF_ERROR(
        PrepareTensor(ctx, a, dims.a_perm, dims.a_batch_size,
                      dims.a_contract_size, need_transpose_a, &a_2d));

    Tensor b_2d;
    TF_RETURN_IF_ERROR(
        PrepareTensor(ctx, b, dims.b_perm, dims.b_contract_size,
                      dims.b_batch_size, need_transpose_b, &b_2d));
  ...
  Status PrepareTensor(OpKernelContext* ctx, const Tensor& input,
    ...
    if (need_transpose) {
      ...
      if (!output->CopyFrom(input, target_shape)) {
        return errors::Internal("Reshape failed. input shape: ",
